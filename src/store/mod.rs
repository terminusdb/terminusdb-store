...
use crate::layer::{
    IdTriple, Layer, LayerBuilder, ObjectLookup, ObjectType, PredicateLookup, StringTriple,
    SubjectLookup,
};
...
use crate::storage::{CachedLayerStore, LabelStore, LayerStore};
...
pub fn open_memory_store() -> Store {
    Store::new(
        ...
        CachedLayerStore::new(MemoryLayerStore::new()),
    )
}
...
pub fn open_directory_store<P: Into<PathBuf>>(path: P) -> Store {
    ...
    Store::new(
        ...
        CachedLayerStore::new(DirectoryLayerStore::new(p)),
    )
}
