...
pub struct TripleStream<S: Stream<Item=(u64,u64),Error=io::Error>+Send> {
...
impl<S:Stream<Item=(u64,u64),Error=io::Error>+Send> TripleStream<S> {
    fn new(s_p_stream: S, sp_o_stream: S) -> TripleStream<S> {
        TripleStream {
...
impl<S:Stream<Item=(u64,u64),Error=io::Error>+Send> Stream for TripleStream<S> {
...
pub fn open_triple_stream<F:'static+FileLoad+FileStore>(s_p_files: AdjacencyListFiles<F>, sp_o_files: AdjacencyListFiles<F>) -> impl Stream<Item=(u64,u64,u64),Error=io::Error>+Send {
    ...
    TripleStream::new(s_p_stream, sp_o_stream)
}

#[cfg(test)]
mod tests {
    use super::*;
    use super::super::base::tests::*;

    #[test]
    fn stream_some_triples() {
        let layer_files = example_base_layer_files();

        let stream = open_triple_stream(layer_files.s_p_adjacency_list_files, layer_files.sp_o_adjacency_list_files);
