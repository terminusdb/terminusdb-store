...
impl<M: 'static + AsRef<[u8]> + Clone + Send + Sync> Layer for BaseLayer<M> {
    ...
    fn id_object(&self, id: u64) -> Option<ObjectType> {
        ...
        if corrected_id >= (self.node_dictionary.len() as u64) {
            ...
            if val_id >= (self.value_dictionary.len() as u64) {
                ...
                Some(ObjectType::Value(
                    self.value_dictionary.get(val_id as usize),
                ))
            }
        } else {
            Some(ObjectType::Node(
                self.node_dictionary.get(corrected_id as usize),
            ))
        }
    }
    ...
    fn lookup_subject_current_layer(
        &self,
        subject: u64,
        _parent: Option<Box<dyn SubjectLookup>>,
    ...
    fn lookup_object_current_layer(
        &self,
        object: u64,
        _parent: Option<Box<dyn ObjectLookup>>,
    ...
    fn lookup_predicate_current_layer(
        &self,
        predicate: u64,
        _parent: Option<Box<dyn PredicateLookup>>,
...
impl<M: 'static + AsRef<[u8]> + Clone> SubjectLookup for BaseSubjectLookup<M> {
    ...
    fn lookup_predicate_current(
        &self,
        predicate: u64,
        _parent: Option<Box<dyn SubjectPredicateLookup>>,
