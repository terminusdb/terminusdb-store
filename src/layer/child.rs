...
impl<M: 'static + AsRef<[u8]> + Clone + Send + Sync> Layer for ChildLayer<M> {
    ...
    fn object_value_id(&self, value: &str) -> Option<u64> {
        ...
        if result.is_some() {
            return Some(
                self.parent.node_and_value_count() as u64
                    + self.node_dict_len() as u64
                    + result.unwrap()
                    + 1,
            );
        }
        ...
        while let Some(parent) = parent_option {
            ...
            if result.is_some() {
                return Some(
                    next_parent.unwrap().node_and_value_count() as u64
                        + parent.node_dict_len() as u64
                        + result.unwrap()
                        + 1,
                );
            }
            ...
            if corrected_id >= parent_count {
                ...
                if corrected_id >= current_layer.node_dict_len() as u64 {
                    ...
                    if corrected_id >= current_layer.value_dict_len() as u64 {
                        ...
                        return Some(ObjectType::Value(
                            current_layer.value_dict_get(corrected_id as usize),
                        ));
                    }
                } else {
                    return Some(ObjectType::Node(
                        current_layer.node_dict_get(corrected_id as usize),
                    ));
                }
            } else {
                current_option = current_layer.parent();
    ...
    fn object_additions(&self) -> Box<dyn Iterator<Item = Box<dyn ObjectLookup>>> {
        ...
        Box::new((0..self.pos_objects.len()).map(move |mapped_object| {
            Box::new(cloned.lookup_object_addition_mapped((mapped_object + 1) as u64))
                as Box<dyn ObjectLookup>
        }))
    }
    ...
    fn object_removals(&self) -> Box<dyn Iterator<Item = Box<dyn ObjectLookup>>> {
        ...
        Box::new((0..self.neg_objects.len()).map(move |mapped_object| {
            Box::new(cloned.lookup_object_removal_mapped((mapped_object + 1) as u64))
                as Box<dyn ObjectLookup>
        }))
    }
    ...
    fn lookup_object_addition(&self, object: u64) -> Option<Box<dyn ObjectLookup>> {
        self.pos_objects.index_of(object).map(|index| {
            Box::new(self.lookup_object_addition_mapped((index as u64) + 1))
                as Box<dyn ObjectLookup>
        })
    }
    ...
    fn lookup_object_removal(&self, object: u64) -> Option<Box<dyn ObjectLookup>> {
        self.pos_objects.index_of(object).map(|index| {
            Box::new(self.lookup_object_removal_mapped((index as u64) + 1)) as Box<dyn ObjectLookup>
        })
    }
    ...
    fn lookup_predicate_current_layer(
        ...
        let parent = parent.map(|parent| ChildPredicateLookupParentData {
            parent,
...
impl<M: 'static + AsRef<[u8]> + Clone> Iterator for ChildSubjectIterator<M> {
    ...
    fn next(&mut self) -> Option<Box<dyn SubjectLookup>> {
        ...
        let next_pos_subject = self
            .pos
            .as_ref()
            .map(|pos| {
                if pos.pos < pos.subjects.len() {
                    pos.subjects.entry(pos.pos)
                } else {
                    0
                }
            })
            .unwrap_or(0);
        let next_neg_subject = self
            .neg
            .as_ref()
            .map(|neg| {
                if neg.pos < neg.subjects.len() {
                    neg.subjects.entry(neg.pos)
                } else {
                    0
                }
            })
        ...
        if next_pos_subject != 0
            && (next_parent_subject == 0 || next_pos_subject <= next_parent_subject)
        {
            ...
            pos = Some(AdjacencyStuff {
                ...
                sp_offset: selfpos
                    .s_p_adjacency_list
                    .offset_for(1 + selfpos.pos as u64),
        ...
        if next_neg_subject != 0
            && (next_pos_subject == 0 || next_parent_subject <= next_pos_subject)
            && next_parent_subject == next_neg_subject
        {
            ...
            neg = Some(AdjacencyStuff {
                ...
                sp_offset: selfneg
                    .s_p_adjacency_list
                    .offset_for(1 + selfneg.pos as u64),
...
impl<M: 'static + AsRef<[u8]> + Clone> SubjectLookup for ChildSubjectLookup<M> {
    ...
    fn lookup_predicate_current(
        ...
        if self.pos.is_none() && parent.is_none() {
            ...
            let pos_objects = self.pos.as_ref().and_then(|pos| {
                pos.predicates.iter().position(|p| p == predicate).map(
                    |position_in_pos_predicates| {
                        pos.sp_o_adjacency_list
                            .get(pos.sp_offset + (position_in_pos_predicates as u64) + 1)
                    },
                )
            });
            let neg_objects = self.neg.as_ref().and_then(|neg| {
                neg.predicates.iter().position(|p| p == predicate).map(
                    |position_in_neg_predicates| {
                        neg.sp_o_adjacency_list
                            .get(neg.sp_offset + (position_in_neg_predicates as u64) + 1)
                    },
                )
            });
...
impl<M: 'static + AsRef<[u8]> + Clone> Iterator for ChildPredicateIterator<M> {
    ...
    fn next(&mut self) -> Option<Box<dyn SubjectPredicateLookup>> {
        ...
        let pos_predicate = if self.pos_pos
            < self
                .pos_adjacencies
                .as_ref()
                .map(|pa| pa.predicates.len())
                .unwrap_or(0)
        {
            Some(
                self.pos_adjacencies
                    .as_ref()
                    .unwrap()
                    .predicates
                    .entry(self.pos_pos),
        ...
        let neg_predicate = if self.neg_pos
            < self
                .neg_adjacencies
                .as_ref()
                .map(|no| no.predicates.len())
                .unwrap_or(0)
        {
            Some(
                self.neg_adjacencies
                    .as_ref()
                    .unwrap()
                    .predicates
                    .entry(self.neg_pos),
        ...
        match (next_parent_predicate, pos_predicate, neg_predicate) {
            (Some(parent), Some(pos), neg) => {
                let neg_objects =
                    if parent.predicate() == neg.unwrap_or(0) && parent.predicate() <= pos {
                        let neg_adjacencies = self.neg_adjacencies.as_ref().unwrap();
                        let result = neg_adjacencies
                            .sp_o_adjacency_list
                            .get(neg_adjacencies.sp_offset + (self.neg_pos as u64) + 1);
                        self.neg_pos += 1;
                ...
                let pos_objects = if parent.predicate() >= pos {
                    ...
                    let result = pos_adjacencies
                        .sp_o_adjacency_list
                        .get(pos_adjacencies.sp_offset + (self.pos_pos as u64) + 1);
                    self.pos_pos += 1;
                ...
                let predicate = if parent_predicate <= pos {
                    parent_predicate
                } else {
                    pos
                };
...
impl<M: 'static + AsRef<[u8]> + Clone> Iterator for ChildObjectIterator<M> {
    ...
    fn next(&mut self) -> Option<u64> {
        ...
        let next_pos_object: Option<u64> =
            if self.pos_pos < self.pos_objects.as_ref().map(|po| po.len()).unwrap_or(0) {
                Some(self.pos_objects.as_ref().unwrap().entry(self.pos_pos))
            } else {
                None
            };
        let next_neg_object: Option<u64> =
            if self.neg_pos < self.neg_objects.as_ref().map(|no| no.len()).unwrap_or(0) {
                Some(self.neg_objects.as_ref().unwrap().entry(self.neg_pos))
            } else {
                None
            };
...
impl<M: 'static + AsRef<[u8]> + Clone> ObjectLookup for ChildObjectLookup<M> {
    ...
    fn subject_predicate_pairs(&self) -> Box<dyn Iterator<Item = (u64, u64)>> {
        Box::new(ChildSubjectPredicatePairsIterator::new(
            self.parent(),
            self.pos.clone(),
            self.neg.clone(),
        ))
...
impl ChildSubjectPredicatePairsIterator {
    fn new<M: 'static + AsRef<[u8]> + Clone>(
        parent: Option<&dyn ObjectLookup>,
        pos: Option<ChildObjectLookupAdjacency<M>>,
        neg: Option<ChildObjectLookupAdjacency<M>>,
    ) -> Self {
        Self {
            ...
            pos: pos
                .map(|p| (Box::new(p.iter()) as Box<dyn Iterator<Item = (u64, u64)>>).peekable()),
            neg: neg
                .map(|n| (Box::new(n.iter()) as Box<dyn Iterator<Item = (u64, u64)>>).peekable()),
        }
    }
}
        ...
        if parent.is_some() {
            ...
            if pos.is_none() || parent < pos.unwrap() {
                ...
                while self
                    .neg
                    .as_mut()
                    .and_then(|n| n.peek().map(|n| *n <= parent))
                    .unwrap_or(false)
                {
...
impl<M: 'static + AsRef<[u8]> + Clone> PredicateLookup for ChildPredicateLookup<M> {
    ...
    fn subject_predicate_pairs(&self) -> Box<dyn Iterator<Item = Box<dyn SubjectPredicateLookup>>> {
        ...
        Box::new(ChildPredicateLookupSubjectPredicatePairsIterator {
            ...
            parent: self.parent.as_ref().map(|p| {
                ChildPredicateLookupSubjectPredicatePairsIteratorParentData {
            ...
            child: self.child.as_ref().map(|child| {
                ChildPredicateLookupSubjectPredicatePairsIteratorChildData {
                    lookup_positions: (Box::new(child.lookup.iter())
...
impl<M: AsRef<[u8]> + Clone> ChildPredicateLookupSubjectPredicatePairsIteratorParentData<M> {
    ...
    fn next(
        ...
        let entry_in_sp_o = mapped_subject.and_then(|mapped_subject| {
            let offset = self
                .neg_s_p_adjacency_list
                .offset_for(mapped_subject as u64);
            let s_p_slice = self.neg_s_p_adjacency_list.get(mapped_subject as u64);
            s_p_slice
                .iter()
                .position(|p| p == pair.predicate())
                .map(|pos_in_slice| offset + (pos_in_slice as u64) + 1)
        });
        let slice =
            entry_in_sp_o.map(|entry_in_sp_o| self.neg_sp_o_adjacency_list.get(entry_in_sp_o));
...
impl<M: AsRef<[u8]> + Clone> ChildPredicateLookupSubjectPredicatePairsIteratorChildData<M> {
    fn peek_next_subject(&mut self) -> Option<u64> {
        self.lookup_positions.peek().map(|&pos| pos).map(|pos| {
...
impl<M: 'static + AsRef<[u8]> + Clone> Iterator
    for ChildPredicateLookupSubjectPredicatePairsIterator<M>
{
    type Item = Box<dyn SubjectPredicateLookup>;
